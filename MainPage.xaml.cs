...
namespace FaceDetection
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private void SetFacesCanvasRotation()
        {
            ...
            myRotateTransform.Angle = rotationDegrees;
            img.RenderTransform = myRotateTransform;
        ...
        private void HighlightDetectedFaces(IReadOnlyList<DetectedFace> faces)
        {
            ...
            for (int i = 0; i < faces.Count; i++)
            {
                ...
                var img = PutImage(faceBoundingBox.Width, faceBoundingBox.Height, faces[i].FaceBox.X, faces[i].FaceBox.Y);
                FacesCanvas.Children.Add(img);
            }
        ...
        Image img = new Image()
        {
            Source = new BitmapImage(new Uri("ms-appx:///Assets/image.jpg")),
        };
        private Image PutImage(double width, double height, double X, double Y)
        {
            ...
            // Scale the X and Y coordinates from preview stream coordinates to window coordinates
            var x = (X / streamWidth) * previewInUI.Width;
            var y = (Y / streamHeight) * previewInUI.Height;
